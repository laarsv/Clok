...
def start_timer(
    ...
    open_entry = db.query(TimeEntry).filter(
        TimeEntry.user_id == user.id, TimeEntry.end_at.is_(None)
    ).first()
    if open_entry:
        raise HTTPException(status_code=409, detail="Es läuft bereits ein Eintrag.")
...
@router.post("/stop", response_model=TimeEntryCreateResponse)
def stop_timer(
    break_minutes: int = 0,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    ...
    if not entry:
        raise HTTPException(status_code=404, detail="Kein laufender Eintrag.")

    entry.end_at = datetime.utcnow()
    ...
    issues = _validate(db, user, payload, exclude_id=entry.id)
